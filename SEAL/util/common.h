...
namespace seal
{
    namespace util
    {
        const int bytes_per_uint64 = sizeof(uint64_t);

        const int bytes_per_uint32 = sizeof(uint32_t);
        ...
        const uint64_t uint64_high_bit = static_cast<uint64_t>(1) << (bits_per_uint64 - 1);
        ...
        inline uint8_t *get_uint64_byte(uint64_t *value, int byte_index)
        {
#ifdef _DEBUG
            if (value == nullptr)
            {
...
#else
            return reinterpret_cast<uint8_t*>(value) + byte_index;
#endif
        }

        inline const uint8_t *get_uint64_byte(const uint64_t *value, int byte_index)
        {
#ifdef _DEBUG
            if (value == nullptr)
            {
...
#else
            return reinterpret_cast<const uint8_t*>(value) + byte_index;
#endif
        }

        inline uint32_t *get_uint64_uint32(uint64_t *value, int uint32_index)
        {
#ifdef _DEBUG
            if (value == nullptr)
            {
...
#else
            return reinterpret_cast<uint32_t*>(value) + uint32_index;
#endif
        }

        inline const uint32_t *get_uint64_uint32(const uint64_t *value, int uint32_index)
        {
#ifdef _DEBUG
            if (value == nullptr)
            {
...
#else
            return reinterpret_cast<const uint32_t*>(value) + uint32_index;
#endif
        }

        int get_significant_bit_count(uint64_t value);

        std::string uint64_to_hex_string(const uint64_t *value, int uint64_count);

        class MemoryPool;
        std::string uint64_to_dec_string(const uint64_t *value, int uint64_count, MemoryPool &pool);
