...
namespace seal
{
    ...
    Simulation SimulationEvaluator::add_many(const vector<Simulation> &simulations)
    {
        ...
        uint64_t scaling_coeff = static_cast<uint64_t>(sqrt((double)(simulations.size())) + 1);
    ...
    Simulation SimulationEvaluator::relinearize(const Simulation &simulation)
    {
        ...
        *decomposition_coeff.get() = 1;
        ...
        temp1 = growth_factor * static_cast<uint64_t>(sqrt((double)simulation.poly_modulus_coeff_count_ - 1) * simulation.noise_max_deviation_ * (simulation.coeff_modulus_.significant_bit_count() / simulation.decomposition_bit_count_ + 2));
    ...
    {
        uint64_t growth_factor = static_cast<uint64_t>(sqrt((double)(2 * (poly_modulus_coeff_count_ - 1) / 3)));
