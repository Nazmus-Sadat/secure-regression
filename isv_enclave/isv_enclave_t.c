...
typedef struct ms_transferMatrix_t {
	char* ms_input;
	char* ms_Result;
	int ms_len_matrix;
	int ms_len_Result;
...
typedef struct ms_transferMatrixXtY_t {
	char* ms_input;
	double* ms_Beta;
	int ms_len_matrix;
} ms_transferMatrixXtY_t;
...
static sgx_status_t SGX_CDECL sgx_transferMatrix(void* pms)
{
	ms_transferMatrix_t* ms = SGX_CAST(ms_transferMatrix_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_input = ms->ms_input;
	int _tmp_len_matrix = ms->ms_len_matrix;
	size_t _len_input = _tmp_len_matrix;
	char* _in_input = NULL;
	char* _tmp_Result = ms->ms_Result;
	int _tmp_len_Result = ms->ms_len_Result;
	size_t _len_Result = _tmp_len_Result;
	char* _in_Result = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_transferMatrix_t));
	...
	transferMatrix(_in_input, _in_Result, _tmp_len_matrix, _tmp_len_Result);
err:
...
static sgx_status_t SGX_CDECL sgx_transferMatrixXtY(void* pms)
{
	ms_transferMatrixXtY_t* ms = SGX_CAST(ms_transferMatrixXtY_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_input = ms->ms_input;
	int _tmp_len_matrix = ms->ms_len_matrix;
	size_t _len_input = _tmp_len_matrix;
	char* _in_input = NULL;
	double* _tmp_Beta = ms->ms_Beta;
	size_t _len_Beta = 4 * sizeof(*_tmp_Beta);
	double* _in_Beta = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_transferMatrixXtY_t));
	CHECK_UNIQUE_POINTER(_tmp_input, _len_input);
	CHECK_UNIQUE_POINTER(_tmp_Beta, _len_Beta);
	...
	if (_tmp_Beta != NULL) {
		if ((_in_Beta = (double*)malloc(_len_Beta)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_Beta, 0, _len_Beta);
	}
	transferMatrixXtY(_in_input, _in_Beta, _tmp_len_matrix);
err:
	if (_in_input) free(_in_input);
	if (_in_Beta) {
		memcpy(_tmp_Beta, _in_Beta, _len_Beta);
		free(_in_Beta);
	}
...
SGX_EXTERNC const struct {
	...
	9,
	{
		...
		{(void*)(uintptr_t)sgx_transferMatrix, 0},
		{(void*)(uintptr_t)sgx_transferMatrixXtY, 0},
