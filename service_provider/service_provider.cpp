...
//uint8_t g_secret[8] = {0,1,2,3,4,5,67,77};


char* mysecret = "5-4796803555973356791818674510632926892407882586692588151734953690210800008847361929847747957286738245101626609080248778027443254771400903147580088452877004";
//char* mysecret = "5707460984083772056065467779121907419296662935268554166562272604932237720508588592700121881153232485200672493393341340028567734931478418511610610340986454";
BigPoly public_key;
                        ...
                        ra_samp_response_header_t **pp_att_result_msg)
{
    ...
    do
    {
        ...
        p_att_result_msg_full =
			...
			+ sizeof(ra_samp_response_header_t) + 15000);
		...
		//encryptorObj = Encryptor(parms, public_key);

        uint8_t aes_gcm_iv[SAMPLE_SP_IV_SIZE] = {0};
        p_att_result_msg->secret.payload_size = 15000;
        if((IAS_QUOTE_OK == attestation_report.status) &&
           ...
           (isv_policy_passed == true))
        {
            ret = sample_rijndael128GCM_encrypt(&g_sp_db.sk_key,
                        //&g_secret[0],
						(uint8_t*)generator.secret_key().to_string().c_str(),
		                ...
		                //(p_att_result_msg->secret.payload + p_att_result_msg->secret.payload_size));
        }
    }while(0);
