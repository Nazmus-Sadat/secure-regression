...
sgx_status_t transferMatrix(sgx_enclave_id_t eid, char* input, char* Result, int len_matrix, int len_Result)
{
	sgx_status_t status;
	ms_transferMatrix_t ms;
	ms.ms_input = input;
	ms.ms_Result = Result;
	ms.ms_len_matrix = len_matrix;
	ms.ms_len_Result = len_Result;
	status = sgx_ecall(eid, 4, &ocall_table_isv_enclave, &ms);
	return status;
}

sgx_status_t transferMatrixXtY(sgx_enclave_id_t eid, char* input, double Beta[4], int len_matrix)
{
	sgx_status_t status;
	ms_transferMatrixXtY_t ms;
	ms.ms_input = input;
	ms.ms_Beta = (double*)Beta;
	ms.ms_len_matrix = len_matrix;
